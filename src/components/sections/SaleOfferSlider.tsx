...
export function SaleOfferSlider({ compact = false }: Props) {
  ...
  if (compact) {
    return (
      <section className="relative h-full min-h-[360px] overflow-hidden rounded-2xl border border-red-200/60 bg-[#fff8f5] shadow-sm">
        {/* Atmosphere */}
        ...
        <div className="relative flex h-full min-h-[360px] flex-col p-5">
          {/* Header */}
          ...
          {/* Book stage */}
          <div className="relative mx-auto flex flex-1 items-center justify-center py-2">
              ...
              >
                <Link to={`/books/${book.slug}`} className="block">
                  <img
                    src={book.cover}
                    alt={book.title}
                    className="h-[168px] w-auto rounded-lg object-cover shadow-[0_20px_40px_-12px_rgba(120,20,20,0.45)] ring-1 ring-black/10 transition-transform duration-300 hover:-translate-y-1 hover:rotate-0"
                  />
                </Link>
              </motion.div>
            </AnimatePresence>
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="mt-1 space-y-3 text-center"
            >
              <div>
                <h2 className="font-serif text-lg font-bold leading-snug text-[hsl(var(--deep-brown))] line-clamp-2">
                  {book.title}
                </h2>
              ...
              <Link
                to={`/books/${book.slug}`}
                className="inline-flex w-full items-center justify-center gap-1.5 rounded-xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-red-600/20 transition-all hover:bg-red-700 hover:shadow-lg active:scale-[0.98]"
              >
                Grab the Deal
